<template>
  <el-dialog :append-to-body="true" :before-close="cancel" :visible.sync="dialog" :title="title" width="900px">
          ...
          <el-form-item :key="index">
            <el-row :gutter="24">
              <template v-for="(item,index) in attr.detail">
                <el-col :span="3" :key="index" style="margin-right: 2px">
                  {{ index }}:{{ item }}
                </el-col>
              </template>
              <el-col :span="4">
  ...
  methods: {
    getAttrs(id) {
      getAttr(id).then(res => {
        ...
        if (res.Data) {
          this.hidden = true
          this.items = res.Data.attr
          this.attrs = res.Data.value
        } else {
          this.hidden = false
          this.items = [{
            value: '',
    ...
    checkGoods() {
      ...
      this.attrs.map(function(attr) {
        ...
        if (!Object.keys(attr.detail).length) {
          ...
          bool = false
        } else if (attr.stock != parseInt(attr.stock) || attr.stock < 0) {
