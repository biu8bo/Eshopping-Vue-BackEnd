  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" style="width: 100%;">
      ...
      <el-table-column prop="" label="评论图片">
        <template slot-scope="scope">
          <div v-if="scope.row.info.reply.pics">
            <a v-for="pic in handlePic(scope.row.info.reply.pics)" :href="pic" style="color: #42b983" target="_blank">
      ...
      <el-table-column prop="addTime" label="评论时间">
        <template slot-scope="scope">
          <span>{{ formatTime(new Date(scope.row.info.reply.create_time)) }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          ...
          >
            ...
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="$refs[scope.row.info.reply.id].doClose()">取消</el-button>
              <el-button :loading="delLoading" type="primary" size="mini" @click="subDelete(scope.row.info.reply.id)">确定</el-button>
            </div>
